...
├── archive_nopass.bat            # 圧縮用バッチファイル（パスワード無効化版）
...
**「送る」メニューでパスワード無効化オプションを使う方法**

「送る」メニューに登録したショートカットで引数を使うには、ショートカットのプロパティを編集します：

1. 「送る」フォルダ内の`archive.bat`のショートカットを右クリック→「プロパティ」を選択
2. 「ショートカット」タブの「リンク先」を編集します
   - 通常：`C:\app\7-zip_helper\archive.bat`
   - パスワード無効化オプション付き：`C:\app\7-zip_helper\archive.bat --no-password`
   - または：`C:\app\7-zip_helper\archive.bat -nopass`
3. 「OK」をクリックして保存

これで、「送る」メニューから実行する際も、パスワード無効化オプションが適用されます。

**複数のショートカットを作成する方法**

パスワードありとパスワードなしの両方を使い分けたい場合：

**方法1：`archive_nopass.bat`を使用（推奨）**

1. 「送る」フォルダに`archive.bat`と`archive_nopass.bat`のショートカットをそれぞれ作成
2. ショートカット名を「archive.bat（パスワードあり）」と「archive_nopass.bat（パスワードなし）」に変更

**方法2：`archive.bat`のショートカットで引数を設定**

1. 「送る」フォルダに`archive.bat`のショートカットを2つ作成
2. 1つ目のショートカット名を「archive.bat（パスワードあり）」に変更、リンク先は通常のまま
3. 2つ目のショートカット名を「archive.bat（パスワードなし）」に変更、リンク先に`--no-password`を追加

これで、「送る」メニューから選択して使い分けられます。
...
設定ファイルにパスワードが設定されている場合でも、パスワードなしで圧縮したい場合は`archive_nopass.bat`を使用してください：

- **ドラッグ&ドロップ**: ファイルまたはフォルダを`archive_nopass.bat`にドラッグ&ドロップ
- **「送る」メニュー**: `archive_nopass.bat`のショートカットを「送る」フォルダに配置
- **コマンドプロンプト**: `archive_nopass.bat "圧縮対象のパス"`

`archive_nopass.bat`は常にパスワード無効化オプションが有効になっているため、設定ファイルにパスワードが設定されていてもパスワードなしで圧縮されます。

**パスワード無効化オプション**

設定ファイルにパスワードが設定されている場合でも、引数でパスワードを無効化できます。

**コマンドプロンプトから実行する場合：**

```batch
archive.bat "圧縮対象のパス" --no-password
```

または

```batch
archive.bat "圧縮対象のパス" -nopass
```
...
**注意：** ドラッグ&ドロップの場合は引数を追加できないため、パスワード無効化オプションは使用できません。コマンドプロンプトから実行するか、「送る」メニューのショートカットで引数を設定してください（上記の「送る」メニューでパスワード無効化オプションを使う方法を参照）。
